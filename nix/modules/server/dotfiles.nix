...
in
{
  ...
  home.activation.cleanupOldConfigLink = lib.hm.dag.entryBefore [ "linkGeneration" ] ''
    ...
    if [[ -L "${configHome}" && ! -d "${configHome}" ]]; then
      ...
      echo ""
      echo "以下のコマンドを実行してから、再度 home-manager switch を実行してください:"
      echo "  rm ${configHome}"
      echo ""
      echo "または、環境変数を設定して自動削除することもできます:"
      echo "  DOTFILES_CLEANUP_CONFIG=yes home-manager switch --flake ~/dotfiles#server"
      echo ""
  ...
  # Server dotfile symlinks (excludes desktop-specific configs)
  home.activation.linkDotfilesServer = lib.hm.dag.entryAfter [ "linkGeneration" ] ''
    ${helpers.mkLinkForce}
    ...
    if [ -d "${dotfilesDir}/zellij" ]; then
      link_force "${dotfilesDir}/zellij" "${configHome}/zellij"
    fi

    # lnav configuration
    if [ -d "${dotfilesDir}/lnav" ]; then
      link_force "${dotfilesDir}/lnav" "${configHome}/lnav"
    fi

    echo ""
    echo "✓ Server dotfiles のシンボリックリンクを作成しました"
    echo ""
